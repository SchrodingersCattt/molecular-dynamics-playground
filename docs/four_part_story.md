...
| 04 Deep Potential MD | `product/figures/04_deep_potential_md.png` / `.svg` | `product/videos/04_deep_potential_md.mp4` | 64-water periodic box, center atom O126, 83 atomic minimum-image neighbors inside 6.0 Å |
...
- The Deep Potential case was evaluated with `H2O-Phase-Diagram-model_compressed.pb` and DeepMD-kit 3.1.3 in a time-limited Bohrium sandbox. The retained 192-atom result gives `E = -1007.9069226533904 eV`, `max |F| = 1.2246452541839372 eV Å⁻¹`, and numerically zero net force. The displayed propagation is a reproducible frozen-force Velocity–Verlet step from that exact snapshot (the second force call is not claimed); the approximation is recorded in `product/qa/04_dpmd_native/vv_metadata.json`.
...
python scripts/md_visuals/render_dpmd_native.py
...
- the DP PPT still and video use a square periodic-box layout; the real O126 neighbor matrix and radial descriptor are recorded in `product/qa/04_dpmd_native/descriptor_provenance.json`.
