...
Model: H2O-Phase-Diagram-model_compressed.pb  (54 MB, pre-downloaded locally)
  ...
  Must be present at md_workflows/H2O-Phase-Diagram-model_compressed.pb before running.
  (The Bohrium cluster may not have internet access; the file is uploaded via
   dpdispatcher forward_files.)

Usage:
    python md_workflows/compute_deepmd_md.py           # submit to Bohrium
    python md_workflows/compute_deepmd_md.py --force   # re-submit even if npz exists

Auth:
    Copy md_workflows/.env.template → md_workflows/.env and fill in BOHR_TICKET
...
def run_bohrium():
    ...
    if not os.path.exists(env_path):
        sys.exit(
            ...
            f"Copy md_workflows/.env.template → md_workflows/.env and fill in credentials."
        )
    try:
    ...
    if not ticket and not (email and password):
        sys.exit(
            ...
            "Set BOHR_TICKET (or BOHR_EMAIL + BOHR_PASSWORD) in md_workflows/.env"
        )
    ...
    if not os.path.exists(WORKER_PATH):
        sys.exit(
            ...
            "Expected: md_workflows/worker_deepmd.py"
        )
