...
本轮只修改了 report/md_reading_notes.tex，并新增本记录文件；没有修改正式视觉产物或视觉脚本，也没有提交或推送。

主要改动：

1. 在初值问题后加入两个短例框。其一直接读取 product/data/vv_h2o_step.npz 的真实首步（\(\Delta t=0.5\) fs、氧原子 \(x\) 分量的位置/速度/加速度和能量变化）；其二是明确标注为“非模拟结果”的一维谐振子量纲算例（\(q_0=1.0\) Å、\(v_0=0.2\) Å/fs、\(a_0=-0.5\) Å/fs²、\(\Delta t=0.1\) fs）。
