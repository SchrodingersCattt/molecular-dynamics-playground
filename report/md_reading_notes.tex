...
\begin{abstract}
分子动力学从给定位置、速度和受力出发，通过离散时间推进得到多体轨迹。轨迹的可信度同时取决于三个相互衔接的问题：积分器能否在长时间传播中保留哈密顿（Hamilton）动力学的几何结构，势能模型能否给出可靠且连续的能量与力，以及有限而相关的轨迹能否支持统计结论。斜抛运动和谐振子分别提供闭式基准与失稳反例，由此可辨认显式 Euler 的相空间膨胀以及速度 Verlet（velocity Verlet）的对称、辛和时间可逆结构。势能来源决定每一步的主要成本：经验势把物理假设写入解析函数，从头算分子动力学（ab initio molecular dynamics, AIMD）在每个离子步内迭代电子密度，深度势（Deep Potential）以局域环境和原子能分解近似电子结构势能面。系综控制、自相关和增强采样进一步决定轨迹访问哪些状态，以及其中包含多少独立信息。积分误差、势能面误差、系综偏差和统计误差需要分层判断；稳定的能量曲线本身不足以说明轨迹能够支持所报告的物理结论。
\end{abstract}

\noindent\textbf{关键词：} 分子动力学；velocity Verlet；势能面；自洽场（SCF）；Deep Potential；增强采样
...
\begin{quote}\small
\textbf{一个带单位的首步例。} 数据文件
\texttt{four\_part\_story/data/vv\_h2o\_step.npz} 给出一个确定性的水分子 velocity Verlet 步，\(\Delta t=0.5\,{\rm fs}\)。以氧原子的 \(x\) 分量为例，初态为
\(r_{0x}=0\,{\rm \AA}\)、\(v_{0x}=0.00331795\,{\rm \AA\,fs^{-1}}\)、\(a_{0x}=0\,{\rm \AA\,fs^{-2}}\)。先更新位置：
\[
r_{1x}=r_{0x}+v_{0x}\Delta t+\tfrac12a_{0x}\Delta t^2
       =0.00165898\,{\rm \AA}.
\]
在这个新位置评价力，得到 \(a_{1x}=2.41289\times10^{-5}\,{\rm \AA\,fs^{-2}}\)，再更新速度：
\[
v_{1x}=v_{0x}+\tfrac12(a_{0x}+a_{1x})\Delta t
       =0.00332399\,{\rm \AA\,fs^{-1}}.
\]
文件中的位置和速度分别为 \(0.00165898\,{\rm \AA}\) 与 \(0.00332399\,{\rm \AA\,fs^{-1}}\)，说明三步确实使用了“旧加速度--新位置--新加速度”的顺序。单步能量差（若记录）也不能单独作为长期能量漂移的判据；这里重点核对的是更新顺序和单位。
\end{quote}

\begin{quote}\small
\textbf{纯教学算例（非本文模拟结果）。} 若一维谐振子满足 \(a(q)=-0.5\,q\,{\rm fs^{-2}}\)，取 \(q_0=1.0\,{\rm \AA}\)、\(v_0=0.2\,{\rm \AA\,fs^{-1}}\)、\(\Delta t=0.1\,{\rm fs}\)，则 \(a_0=-0.5\,{\rm \AA\,fs^{-2}}\)。一行 velocity Verlet 更新给出
\[
q_1=1.0+0.2(0.1)+\tfrac12(-0.5)(0.1)^2=1.0175\,{\rm \AA},
\quad a_1=-0.50875\,{\rm \AA\,fs^{-2}},
\]
\[
v_1=0.2+\tfrac12(-0.5-0.50875)(0.1)
   =0.1495625\,{\rm \AA\,fs^{-1}}.
\]
这个例子只展示量纲和更新顺序，不是仓库中任何轨迹的测量值。
\end{quote}
...
把式~\eqref{eq:newton}直接按一阶 Taylor 展开，可得到显式 Euler（explicit Euler）：
\begin{equation}
  ...
  \label{eq:euler}
\end{equation}
位置和速度都只使用旧时刻信息，这一点很关键。若先更新速度，再用新速度更新位置，得到的是辛 Euler（symplectic Euler）；它仍是一阶方法，却具有不同的长期几何性质，不能与式~\eqref{eq:euler}混称。三种方法的边界先说清楚：显式 Euler 适合教学反例和极短步调试，不适合长时间保守动力学；辛 Euler 适合可分 Hamiltonian 的保守部分，能保持辛结构但只有一阶精度且位置、速度的更新有偏；velocity Verlet 适合固定步长、光滑保守力的微正则（NVE）传播。加入恒温器（thermostat）/恒压器（barostat）、非光滑截断、未收敛电子力或不相容约束后，后两者的严格辛性和可逆性不再自动成立。
...
\end{itemize}
Velocity Verlet 的长期优势来自后向误差分析：在足够小的步长下，离散轨迹可以理解为某个邻近的 shadow Hamiltonian（影子哈密顿量）的精确轨迹，因此相对于该修正能量的偏差通常有界；原 Hamiltonian 的误差常表现为有界振荡，但不必严格围绕零，也不等于逐步精确守恒\cite{hairer2006}. “能量表现好”是这种结构的结果，不应被简化成“Verlet 精确守恒能量”。
...
\begin{align}
  ...
  \label{eq:vv-v}
\end{align}
真正昂贵的通常是式~\eqref{eq:vv-a}：新位置确定后必须重新评价势能及其梯度。式~\eqref{eq:vv-r}--\eqref{eq:vv-v}因此也自然形成“位置--加速度--速度”的三节点循环。若下一步继续推进，$\bm A_{n+1}$ 可直接复用，每个时间步只需一次新的力计算。这里的“辛、可逆、二阶”仍以固定步长、可分 Hamiltonian 和一致的保守力为前提；温控、压控、约束、邻居表跳变或电子 SCF 截断会改变这些严格性质。
...
\begin{table}[htbp]
\centering
\caption{常见固定步长积分器的性质（保守、可分 Hamiltonian 的 NVE 条件）。这里“力/步”按无额外复用的一般实现计；具体程序可复用端点力。}
\small
...
常见显式 RK2 & 2 & 通常否 & 通常否 & 2 & 可出现长期漂移 & 一般 ODE \\
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.96\textwidth]{../_archive/legacy/integrator_animations/figures/integrator_comparison.pdf}
...
水模型说明了“一个 LJ 项”与“完整力场”的区别。TIP3P 等固定电荷模型通常在氧--氧之间使用 LJ 项，同时包含原子部分电荷间的静电作用，并以键长、键角约束或势项维持分子内几何\cite{jorgensen1983}. 因此在水二聚体上突出 $r_{\mathrm{OO}}\to U_{\mathrm{LJ}}\to F\to a$ 是一种局部解释，不应暗示氢键完全由 LJ 描述。图~\ref{fig:vv-lj} 及其视频把每个水分子暂作一个刚体平移的广义坐标，只显示 O--O LJ 分量；它不是完整 TIP3P 的转动、约束和静电动力学。经验势速度快，是因为函数形式和参数已经把大量电子结构响应压缩掉；它的局限也来自同一处：电荷转移、成键断键、极化和跨化学空间外推通常需要额外模型或重新参数化。
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.485\textwidth}
    \includegraphics[width=\linewidth]{../product/figures/01_velocity_verlet.png}
    \caption{Velocity Verlet：抽象三步与刚体水分子示例（位移箭头为显示放大）。}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.485\textwidth}
    \includegraphics[width=\linewidth]{../product/figures/02_classical_lj.png}
    \caption{经典势：水二聚体上的 O--O LJ 分量（非完整 TIP3P 力）。}
  \end{subfigure}
  \caption{Velocity Verlet 与经验势在水体系中的具体对应。左图显示位置、加速度和速度的离散更新及一个刚体水分子的示例位移；右图只以水二聚体的 O--O 距离说明 LJ 分量的解析势能、力与加速度关系，静电和分子内项未显示。}
  \label{fig:vv-lj}
\end{figure}

\subsection{AIMD 的电子自洽力}

从头算分子动力学不预先拟合固定解析势，而是在每个离子构型上求电子基态能量，再由电子能量的核梯度推进原子。Born--Oppenheimer 近似利用电子与原子核质量尺度差异，把核在一张电子基态势能面上的运动与电子问题分开\cite{born1927}. 对 Hartree--Fock 或 Kohn--Sham DFT，电子方程又是非线性的：轨道决定密度，密度决定有效势或 Fock 矩阵，而它们反过来决定新轨道。

图~\ref{fig:aimd} 的具体数据是中性、闭壳层水二聚体的限制 Hartree--Fock（restricted Hartree--Fock, RHF）/STO-3G 基组教学例（总电荷 0、单重态），不是生产级 DFT 结果。核力由每个收敛 RHF 总能量的三维中心有限差分得到，力的记录单位为 $E_h/\mathrm{Bohr}$；因此这里不把数值梯度称作“纯 Hellmann--Feynman 力”，有限基组的基函数响应（Pulay 项）已随能量差分进入结果。为避免符号冲突，下文用 $\mathbf F_{\mathrm{Fock}}$ 表示 Fock 矩阵，用 $\bm F_i$ 表示第 $i$ 个原子的核力。

一个自洽场（self-consistent field, SCF）循环可以写成
\begin{equation}
  P^{(k)} \xrightarrow{\text{构造 Fock}}
  \mathbf F_{\mathrm{Fock}}[P^{(k)}] \xrightarrow{\text{解本征问题}}
  C^{(k+1)} \xrightarrow{\text{占据}}
  P_{\mathrm{out}}^{(k+1)}
  \xrightarrow{\text{混合}} P^{(k+1)}.
\end{equation}
迭代中的 $P$ 是原子轨道基组中的电子密度矩阵，$C$ 是轨道系数，$\mathbf F_{\mathrm{Fock}}$ 是由当前密度构造的 Fock 矩阵。Roothaan 方程奠定了基组中的闭壳层 Hartree--Fock 自洽形式\cite{roothaan1951}；DIIS 等外推方法通过残差子空间加速并稳定收敛\cite{pulay1980}. 重要的是，SCF 总能量并不保证每次迭代单调下降；混合、阻尼和 DIIS 可能造成局部回升。

图~\ref{fig:aimd} 右上角的曲线应按两个量来读。其纵轴
\begin{equation}
  e_E^{(k)}=\frac{|E^{(k)}-E^\star|}{E_h}
\end{equation}
是同一离子构型中第 $k$ 次迭代能量与最终能量 $E^\star$ 的距离，不是 SCF 残差。数据文件同时保存了密度更新残差
\begin{equation}
  r_\rho^{(k)}=\frac{\lVert P_{\mathrm{out}}^{(k)}-P^{(k)}\rVert_F}{\sqrt{N_{\mathrm{AO}}^2}}.
\end{equation}
本示例的停止判断为相邻总能量差小于 $10^{-8}E_h$；$r_\rho$ 作为独立诊断，生产计算还应同时检查密度（或交换子）和核力阈值。现有七个几何的 SCF 计数为 $12,11,11,11,11,11,11$，末次 $r_\rho$ 为 $9.42\times10^{-9}$--$3.62\times10^{-8}$，这些数字说明能量平台与密度自洽不是同一个条件。
...
二维密度等值图比三维等值面更适合水二聚体的教学动画。图中每条等值线来自固定分子平面和固定网格上、由第 $k$ 个 AO 密度矩阵评估的真实电子密度 $\rho^{(k)}(\bm r)$；现有数据的平面数组为 $280\times420$，坐标范围由数据文件给出。为避免迭代帧闪烁，视频另对该真实字段施加一个随 $r_\rho^{(k)}$ 单调变化的 Gaussian blur；模糊半径是视觉编码，不是电子云的物理宽度。因而等值线的绝对数值、单位和色标应固定并标出，不能把“变清晰”本身当作收敛证据。收敛后停顿，再分阶段显示核梯度、速度更新和位置漂移，才把“电子步嵌套在离子步内”的层级说清楚。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.93\textwidth]{../product/figures/03_aimd_scf.png}
  \caption{AIMD 中电子步与离子步的嵌套关系（本图为 RHF/STO-3G 教学示例）。每个离子构型先经过 SCF 迭代得到收敛电子密度和核力，随后更新速度与位置并进入下一离子步；右上曲线是能量误差示意，不等同于密度残差。}
  \label{fig:aimd}
\end{figure}
...
Deep Potential 的局域计算可由同一快照中的数据链展开：中心原子周围 $6$~\AA{} 截断球内的周期最小镜像邻居构成局域环境，环境表示经过共享网络得到该中心的模型原子能。必须遍历快照中的全部中心原子后再求和，才得到总能量；对总能量求梯度才得到全局各原子力。图~\ref{fig:dpmd} 中的 $6$~\AA{} 和 $83$ 个邻居来自一个边长 $12.4296$~\AA{}、$64$ 个刚性水分子的示例盒，不是普适常数。单个 $E_i$ 是模型分解而非可独立测量的原子结合能，只有总能量、力及其验证误差具有本文所需的物理解释；中心原子、邻居列表、原子能、总能量和力也必须来自同一次计算，才能保持映射的一致性。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.93\textwidth]{../product/figures/04_deep_potential_md.png}
  \caption{Deep Potential MD 中从局域环境到全局力的映射（示意）。每个中心原子的截断邻域分别经过共享模型得到 $E_i$，遍历全部原子后求和，最后对总能量求梯度并返回外层积分器；图中 $r_c=6$~\AA{}、83 个邻居和能量数值仅对应示例水盒。}
  \label{fig:dpmd}
\end{figure}
...
对时间序列 $A_t$，样本之间通常相关。令 $N$ 为保存帧数、$\Delta t$ 为帧间隔，归一化自相关函数为 $\rho_k$，则本文采用时间单位的积分自相关时间约定
\begin{equation}
  \tau_{\mathrm{int}}=\Delta t\left(\frac12+\sum_{k\ge1}\rho_k\right),
\end{equation}
并在自相关噪声超过零后截断求和。若方差为 $\sigma_A^2$，有效样本数的量级为：
\begin{equation}
...
\begin{quote}\small
\textbf{现有数据的数字例（不是平衡统计结论）。} 文件 \texttt{four\_part\_story/data/aimd\_multistep\_h2o\_dimer.npz} 含 7 个连续几何、6 次离子更新，\(\Delta t=0.5\,{\rm fs}\)，因而总跨度只有 \(3.0\,{\rm fs}\)。其中 7 个收敛构型的总能量均值为 \(-149.934112\,E_h\)，样本标准差为 \(5.28\times10^{-4}\,E_h\)，范围为 \([-149.934954,-149.933515]\,E_h\)。这只是对文件中 7 个相关点的描述性计算；没有平衡段、独立副本或自相关时间，不能把 \(s/\sqrt{7}\) 当作标准误差，也不能据此报告 \(N_{\rm eff}\) 或自由能。另一个 \texttt{water\_box\_64.json} 明确标记水盒为“prepared visualization structure; not MD-equilibrated”，所以本文只把它用作局域环境示例。
\end{quote}

\begin{quote}\small
\textbf{自相关教学算例（非本文数据）。} 若保存 \(N=1000\) 帧、帧间隔 \(\Delta t=1\,{\rm fs}\)，并从独立分析得到 \(\tau_{\rm int}=20\,{\rm fs}\)，则
\[
N_{\rm eff}\simeq\frac{1000\times1}{2\times20}=25.
\]
这 25 不是由上面的七个水二聚体构型推出的；实际工作必须先估计 \(\rho_k\) 和 \(\tau_{\rm int}\)，再报告误差条。
\end{quote}

\subsection{系综控制的动力学时间尺度}

常规采样还要区分积分器与恒温器（thermostat）/恒压器（barostat）。Velocity Verlet 解决确定性 Hamilton 部分；Langevin、Nos\'e--Hoover 链等改变动力学以采样目标温度；barostat 再引入体积或晶胞自由度。不同算法可能给出相同的平衡分布，却有不同的动力学时间尺度。若目标是扩散系数、粘度或时间关联函数，温控耦合强度不能只按“温度曲线最平”选择。

\subsection{自由能势垒的增强采样}

自由能势垒高于热涨落时，普通 MD 会在单个亚稳态中停留很久。增加总步数只有在能覆盖多次独立跃迁时才有效。增强采样通过偏置、约束或副本交换改变访问概率，再把偏置正确地重加权回目标分布。

伞形采样（umbrella sampling）在一组重叠窗口中给集体变量（collective variable, CV）$s(\bm R)$ 加谐和约束，分别采样后用加权直方图分析（WHAM）或多状态 Bennett 接受率（MBAR）等方法拼接自由能\cite{torrie1977}. 它的优点是窗口可并行、覆盖范围明确；困难在于窗口布置、隐藏慢变量和相邻窗口重叠不足。
...
四层误差需要由分层证据区分：小体系 NVE 先检验时间步、总能量有界性和力连续性；固定数值设置后再比较势模型或电子结构层级；随后检查体系尺寸、截断、长程修正和系综参数；最后用独立轨迹、块平均和跨态次数估计统计误差。只有把这四层分开，才能知道“结果变了”究竟该改积分器、势函数还是采样策略。
