...
\definecolor{scarlet}{HTML}{A33A33}
\definecolor{lake}{HTML}{4D96A8}
\definecolor{olive}{HTML}{A7A05B}
\definecolor{emerald}{HTML}{4F8A6B}
...
\hypersetup{
  ...
  citecolor=scarlet,
  urlcolor=lake,
  pdftitle={从轨迹积分到机器学习势：分子动力学的计算主线}
}
...
\title{\textbf{从轨迹积分到机器学习势：分子动力学的计算主线}\\[0.4em]
\large 一份围绕积分器、势能面与采样的读书笔记}
\author{}
\date{\today}
...
\begin{abstract}
分子动力学常被介绍成“让原子动起来”，但真正值得抓住的是一个反复出现的初值问题：给定当前位置、速度和受力，如何可靠地推进轨迹；给定原子坐标，又如何以可承受的成本得到势能和力。本文从斜抛运动出发，依次讨论显式 Euler、辛 Euler、Verlet、velocity Verlet 与固定步长 Runge--Kutta 方法，说明相空间体积保持、辛性、时间可逆性和精确能量守恒并不是同一件事。随后把每个时间步拆成“坐标--势能--力--积分器”闭环，比较经验势、Born--Oppenheimer 从头算分子动力学、Car--Parrinello 方法和机器学习原子间势。Deep Potential 被用作主例，展示局域邻域、对称性、原子能分解以及由能量梯度得到力的逻辑。最后讨论平衡、相关时间、分块平均、有效样本数，以及 umbrella sampling 和 well-tempered metadynamics。全文的重点不是罗列方法，而是说明：积分器决定如何走，势函数决定走在哪一张地形图上，采样理论决定有限轨迹究竟能支持什么统计结论。

\noindent\textbf{关键词：} 分子动力学；velocity Verlet；势能面；自洽场；Deep Potential；增强采样
\end{abstract}

\section{从斜抛运动到多体初值问题}
...
\section{积分器：如何可靠地走下一步}

\subsection{Verlet、leapfrog 与 velocity Verlet}
...
\section{真正昂贵的环节：势能面与力}

所有上述积分器都把力当作可调用的函数。分子动力学的第二条主线因此是
\begin{equation}
...
\subsection{一条更准确的历史线}

Fermi--Pasta--Ulam 1955 年的数值实验研究非线性振子链的能量弛豫，是计算动力学史上的标志性工作，但把它称作第一项标准分子动力学模拟并不严谨\cite{fpu1955}. Alder 与 Wainwright 1957 年对硬球体系的研究通常被列为早期经典 MD 里程碑\cite{alder1957}；Rahman 1964 年用 Lennard--Jones 势模拟液氩，展示了从原子轨迹计算液体结构和动力学关联的路径\cite{rahman1964}；Verlet 1967 年重新组织并推广了后来广泛使用的位置递推形式\cite{verlet1967}. 这段历史的共同点不是“势越来越复杂”，而是轨迹、统计力学和可计算势函数逐渐闭合成一套实践方法。
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{同一积分循环在抽象层和具体结构层之间的对应。图中结构由 MatterVis 渲染；公式环与结构区分开，避免把符号和原子堆叠在同一视觉层。}
  \label{fig:vv-lj}
\end{figure}

\section{AIMD：每个离子步里再嵌一层电子迭代}

\subsection{Born--Oppenheimer 分离与 SCF}
...
\subsection{Born--Oppenheimer MD 与 Car--Parrinello MD}

Born--Oppenheimer MD 在每个离子步把电子自由度收敛到基态，再计算核力。Car--Parrinello MD 则引入电子轨道的虚拟质量，把轨道和离子放在扩展 Lagrangian 中联合传播\cite{carparrinello1985}. 后者不是“省略 SCF 的同一个算法”，而是用绝热分离和受约束电子动力学保持轨道接近瞬时基态。虚拟质量、时间步和电子--离子能量交换必须仔细控制。现代实现中，BOMD 配合高效 SCF、外推和预条件往往更直接；CPMD 仍有重要历史和方法学价值。
...
\begin{figure}[htbp]
  ...
  \caption{AIMD 的双层循环：外层为 simulation step 和 velocity Verlet，右上角只记录当前离子步的 SCF 收敛信息，右下角为电子迭代环；中间结构区保留水二聚体、二维密度、核力和位移。}
  \label{fig:aimd}
\end{figure}

\section{机器学习势：学习势能面，而不是跳过物理}

\subsection{从手写函数到数据驱动表示}
...
一个清楚的可视化应严格使用同一快照的数据链：先显示正常双色化学键；选中中心原子；画出真实 $6$~\AA{} 截断球；保持球内原子和键正常、球外淡化；再把化学键视角切换为带周期最小镜像的邻居向量；最后展示“局域描述符--共享网络--原子能--总能量/力”。中心原子、邻居列表、原子能、总能量和力若来自不同计算结果，画面即使漂亮也失去科学自洽性。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.93\textwidth]{../four_part_story/figures/04_deep_potential_md.png}
  \caption{Deep Potential MD：局域邻域经过共享模型汇总成能量和力，再返回外层积分器。真实三维盒、周期结构、双色化学键、截断球和原子中心向量应由同一结构渲染体系生成。}
  \label{fig:dpmd}
\end{figure}

\subsection{2007 年以后：几条并行发展的路线}

现代 MLIP 很难用一条“后一代全面替代前一代”的序列概括。描述符、核方法、消息传递、等变表示、训练数据组织和部署效率在并行演化。表~\ref{tab:mlip}只给出定位，不把不同数据集上的误差横向硬比。

\begin{longtable}{p{0.12\textwidth}p{0.16\textwidth}p{0.45\textwidth}p{0.10\textwidth}}
...
Deep Potential & 局域环境+共享深网 & 能量、力、并行训练和大规模 MD 的端到端闭环\cite{han2018,zhang2018deepmd} & 已发表 \\
...
一个合适的教学 Demo 是可复现的一维双阱。前五秒让普通 Langevin 动力学停留在单井；后十一秒启动 well-tempered hills，左侧显示粒子在原势与时变偏置之和上运动，右侧只显示已沉积偏置和恢复的自由能。这样能把“加速跨垒”和“恢复无偏自由能”同时说清楚。演示应报告随机种子、时间步、摩擦、温度、hill 高度/宽度、沉积间隔、bias factor、跨阱次数和自由能均方误差；否则它只是一个会动的示意图。
...
\section{从模型到可复现轨迹：一套最小工作流}

\subsection{先定义问题，再生成初态}
...
进入真实体系后，先跑短 NVE。合理现象是能量围绕一个窄区间振荡，缩小步长后振幅按预期下降；持续单向漂移则需要依次排查力不连续、邻居表更新、约束容差、SCF 未收敛和积分器实现。温度短时起伏不是失败，尤其在小体系里；反而是过强温控得到一条过分平滑的温度线，可能掩盖真实动力学。只有 NVE 基线通过后，再引入 thermostat 或 barostat，调节耦合时间并验证目标系综。

\subsection{把力计算器当作独立软件验证}
...
最后应保存一份能够重建结论而不只重播动画的最小档案：初始结构、势模型哈希、软件版本、随机种子、时间步、系综参数、约束、截断和长程设置、SCF 阈值、平衡期判据、生产期范围以及分析脚本。轨迹文件很大时可以降低保存频率或保存关键统计量，但元数据不能省略。可复现性在这里不是附加文档，而是区分物理变化与软件变化的实验条件。
...
因此，理解 MD 不应止于背诵三条 Verlet 公式。真正完整的计算链是：用物理模型定义势能面，用数值积分器推进轨迹，用系综方法规定要采样的分布，再用相关性分析判断有限轨迹包含多少信息。动画和图的价值也在这里：不是为公式加装饰，而是把抽象循环、真实结构、电子迭代和统计收敛放在同一个可核查的因果链上。

\appendix
\section{最小数值自查清单}

\begin{itemize}
  \item 时间网格满足 $t_n=t_0+n\Delta t$，输出帧与积分步不混淆。
  \item 谐振子上 velocity Verlet 的全局误差随 $\Delta t^2$ 收敛。
  \item 显式 Euler 与辛 Euler 的更新矩阵、行列式和代码更新顺序一致。
  \item LJ 解析力与中心有限差分在代表性 $r$ 上一致，并检查截断附近连续性。
  \item SCF 密度积分与电子数一致；记录实际收敛判据，不要求能量逐步单调。
  \item MLIP 的中心原子、最小镜像邻居、能量与力来自同一快照；力与能量梯度一致。
  \item NVE、NVT/NPT 和增强采样分别报告其算法、参数、平衡期和生产期。
  \item 块长达到误差平台；报告 $N_{\mathrm{eff}}$ 或积分自相关时间。
  \item 增强采样报告集体变量定义、偏置参数、跨态次数和自由能误差。
\end{itemize}
